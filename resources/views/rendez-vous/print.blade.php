...
<div id="recu-container" class="a4">
    ...
    <div class="consult-title">REÇU DE RENDEZ-VOUS</div>
    <div class="bloc-patient">
        <table class="bloc-patient-table">
            <tr>
                <td class="label">N° Fiche :</td>
                <td class="value">{{ $rendezVous->patient->NumFiche }}</td>
                <td class="label">Nom Patient :</td>
                <td class="value">{{ $rendezVous->patient->Prenom }}</td>
                <td class="label">Réf :</td>
                <td class="value">{{ str_pad($rendezVous->OrdreRDV, 3, '0', STR_PAD_LEFT) }}</td>
            </tr>
            <tr>
                <td class="label">Praticien :</td>
                <td class="value praticien-value">Dr. {{ $rendezVous->medecin->Nom }} {{ $rendezVous->medecin->Prenom }}</td>
                <td class="label">Date :</td>
                <td class="value">{{ \Carbon\Carbon::parse($rendezVous->dtPrevuRDV)->format('d/m/Y') }}</td>
                <td class="label">Heure :</td>
                <td class="value">{{ \Carbon\Carbon::parse($rendezVous->HeureRdv)->format('H:i') }}</td>
            </tr>
        </table>
    </div>
